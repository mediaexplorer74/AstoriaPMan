...
namespace Astoria_Package_Manager
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        public MainPage()
        {
            try
            {
                ...
                OutputBox.Text = "\n[First Release Candidate]\n\n" +
                    "Open a compatible package to install, bigger packages (60MB+) " +
                    "or large Manifests will take longer to process.\n\n";

                AboutText.Text = $"Version: {APMVersion}\n\n This software uses several " +
                    $"Open Source libraries:\n• Iteedee.ApkReader by hylander0\n" +
                    "• MegaApiClient fork from MediaExplorer\n\n" +
                    "Want to submit an app to the repo?" +
                    "\nRepack/Convert the app to Appx, and upload to the ProjectA Telegram, " +
                    "I will update the repo as needed";
        ...
        private async void GithubAStorageLnk_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                string readmeURL 
                    = "https://github.com/Empyreal96/Astoria-Package-Manager/raw/main/README.txt";
                ...
                var readmecreate = await ApplicationData.Current.LocalFolder.CreateFileAsync(
                    "README.txt", CreationCollisionOption.ReplaceExisting);
                ...
                {
                    ExceptionHelper.Exceptions.CustomException(
                        "Error fetching file \"README.txt\"");
                }
            ...
            {
                ExceptionHelper.Exceptions.CustomException(
                    "Please connect to the Internet to fetch latest Readme");
            }
        }
        ...
        public async void PrintAndroidManifest(TextBlock textblock, string manifest, Stream stream)
        {
            try
            {
                ...
                var imgfile = await ApplicationData.Current.LocalCacheFolder
                    .GetFileAsync("AppLogo.scale-240.png");
                FileRandomAccessStream iconstream = (FileRandomAccessStream)await 
                    imgfile.OpenAsync(FileAccessMode.Read);
                ...
                AndroidAPIs.CheckminAPILevel(minsdk.ToString()
                    .Replace("minSdkVersion=", "").Replace("\"", ""));
                AndroidAPIs.CheckmaxAPILevel(targetsdk.ToString().Replace("targetSdkVersion=", "")
                    .Replace("\"", ""));
        ...
        public static async Task Decompress(StorageFile source, StorageFolder destination)
        {
                                ...
                                if (entry.Name == "payload.apk")
                                {
                                    ...
                                    using (Stream entryStream = entry.Open())
                                    {
                                        ...
                                        try
                                        {
                                            ...
                                            {
                                                using (Stream data = uncompressedFileStream
                                                    .AsStreamForWrite())
                                                {
                                ...
                                if (entry.Name == "AppLogo.scale-240.png")
                                {
                                    ...
                                    using (Stream entryStream = entry.Open())
                                    {
                                        ...
                                        try
                                        {
                                            ...
                                            {
                                                using (Stream data = 
                                                    uncompressedFileStream.AsStreamForWrite())
                                                {
                                                    data.Write(buffer, 0, buffer.Length);
                                                    data.Flush();
                                                }
            ...
            {
                IsAPKFile = true;
                await source.CopyAsync(ApplicationData.Current.LocalFolder, "payload.apk", 
        ...
        public async void DecodeManifest(StorageFolder folder)
        {
            ...
            using (BinaryReader s = new BinaryReader(strm))
            {
                ...
                await FileIO.WriteTextAsync(await folder.CreateFileAsync("AndroidManifest_decoded.xml"), 
                    newmanifest);
                /// FOR DEBUG
                //await SaveManifest();
                ///
                PrintAndroidManifest(OutputBox, newmanifest, 
                    await folder.OpenStreamForReadAsync("AndroidManifest_decoded.xml"));
            }
...
SaveManifest()
        {
            ...
            await saveman.CopyAsync(storage2, "AndroidManifest_decoded.xml", 
        ...
        public async void InstallPackage(StorageFile file)
        {
            if (IsAPKFile == false)
            {
                try
                {
                    ...
                    await pacman.AddPackageAsync(new Uri(file.Path), null,
        ...
        {
            var ArcadiaEXE = await StorageFile.GetFileFromApplicationUriAsync(
                new Uri("ms-appx:///Assets/Arcadia/Template/Arcadia.exe"));
            var ArcadiaWINMD = await StorageFile.GetFileFromApplicationUriAsync(
                new Uri("ms-appx:///Assets/Arcadia/Template/Arcadia.winmd"));
            var AoWBackgroundTaskDLL = await StorageFile.GetFileFromApplicationUriAsync(
                new Uri("ms-appx:///Assets/Arcadia/Template/AoWBackgroundTask.dll"));
            var ArcadiaManifestXML = await StorageFile.GetFileFromApplicationUriAsync(
                new Uri("ms-appx:///Assets/Arcadia/Template/AppxManifest.xml"));
            var ArcadiaConfigXML = await StorageFile.GetFileFromApplicationUriAsync(
                new Uri("ms-appx:///Assets/Arcadia/Template/Config.xml"));
            var MakePriAppDataEXE = await StorageFile.GetFileFromApplicationUriAsync(
                new Uri("ms-appx:///Assets/Arcadia/Tools/MakePri.exe"));
        ...
        public async void ClearStorage()
        {
            ...
            if (tempFiles != null)
            {
                foreach (var tempItem in tempFiles)
                {
                    ...
                    {
                        Debug.WriteLine("[ex] ClearStorage - DeleteAsync Exception: " + ex.Message);
                    }
                }
            }
        }
        ...
        private async void InstallPackageBtn_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (IsAppDataRequired == true)
            {
                try
                {
                   ...
                   var AndroidOutput = await AndroidFolder.CreateFolderAsync(PackageName, 
                    ...
                    foreach (var obb in files)
                    {
                        ...
                        {
                            ExceptionHelper.Exceptions.CustomException(
                                "There was a issue in copying Data");
                            return;
                        }
                        OpenArchiveHeader.Text = $"Copying {obb.Name} Finished";
                        OutputBox.Text = $"Copied {obb.Name} Successfully, " +
                            $"Size: {((long)info.Size).ToFileSize()}";
                    }
           ...
           InstallPackage(SelectedPackage);
            ...
            progressBar.Visibility = Visibility.Collapsed;
            progressBar.IsIndeterminate = false;
            var CompletedInstall = new MessageDialog(
                $"{PackageName} has installed successfully. \n\nIt may take a few seconds to show. " +
        ...
        /// <returns></returns>
        public static async Task<long> CopyFolder(StorageFolder targetFolder, 
            StorageFolder destinationFolder, string customExtention = "")
        {
            long folderSize = 0;
            try
            {
                ...
                {
                    var fileSizeTasks = (await folders.GetFilesAsync()).Where(item => 
                    Path.GetExtension(item.Path).ToLower().Equals(customExtention.ToLower()))
                        .Select(async file => (await file.CopyAsync(destinationFolder, 
                ...
                {
                    var fileSizeTasks = (await folders.GetFilesAsync()).Select(async file => 
